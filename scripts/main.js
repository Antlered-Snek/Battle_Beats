...
// Imports
	// functions.js
import { c, c1, c2, canvas_height, canvas_width, gravity, dashTime, drag, gameFrame, mapFrame, frameUpdate } from './universalVar.js'
import { keys, gamepadHandler } from './inputHandler.js'
import { freezeFrames, rectangularCollision, freezeFrame, freezeFrameReduce, screenShake, getDistance, pause } from './functions.js'
...
let battleCommencing = true;
export let showHitbox = true;
...
function animate() {
	...
	if (battleCommencing) {
		// Global
		showHud = keys.f1.pressed;
		showHitbox = keys.f3.pressed;
		isPaused = !keys.escp.pressed;
		if (!isPaused && pauseIndex != null) {
			mainMenu[pauseIndex].isIrrelevant = true;
			pauseIndex = null;
		}
		else if (isPaused && pauseIndex === null) pauseIndex = pause();
		if (isPaused) freezeFrame(1, false);
		...
		c2.clearRect(0, 0, canvas_width, canvas_height);
		for (let i in menuUI) menuUI[i].update();
		for (let i in mainMenu) mainMenu[i].update();


		if (freezeFrames === 0) {
			c1.clearRect(0, 0, canvas_width, canvas_height);
			c.clearRect(0, 0, canvas_width, canvas_height);
			if (map != null) {
				c1.drawImage(map, 0, (mapFrame%8-1)*1440, 2560, 1440, 0, 0, canvas_width, canvas_height);
			}
			else {
				c1.fillStyle = "black";
				c1.fillRect(0, 0, canvas_width, canvas_height);
			}
